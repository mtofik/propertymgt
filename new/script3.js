document.addEventListener("DOMContentLoaded", function () {
    ...
    document.getElementById("userRegistrationForm").addEventListener("submit", function(e){
        e.preventDefault();
        fetch(`http://localhost:3000/student/update`, {
            method:'put',
            headers:{'content-type':'application/json'},
            body:JSON.stringify({id: id.value, name: name.value, age: age.value})
        } )
        .then(function(response){
            dbStatus.innerHTML = `
                <span style="color: green"><i class="fas fa-check-circle"></i> Student updated successfully.</span>
                `;
